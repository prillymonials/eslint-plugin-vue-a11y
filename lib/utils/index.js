...
module.exports = {
  ...
  isDOMElement (tagName) {
    const domElements = [...dom.keys()];
    return domElements.indexOf(tagName) > -1;
  },
  ...
  hasLabellingProp(that, openingElement, additionalLabellingProps = []) {
    const { getAttributeName, getAttributeValue } = that;
    ...
    return openingElement.attributes.some((attribute) => {
      ...
      if (
        labellingProps.indexOf(attrName) > -1
        && !!attrValue
      ) {
        return true;
      }
  ...
  mayHaveAccessibleLabel(root, maxDepth = 1, additionalLabellingProps = []) {
    const that = this;
    function checkElement(node, depth) {
      ...
      if (
        node.startTag
        && that.hasLabellingProp(that, node.startTag, additionalLabellingProps)
      ) {
        return true;
      }
